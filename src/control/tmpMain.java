...
//		u.addExcursion((Excursion) us);
		...
		try {
			u.addExcursion(new UscitaSquadrigliaImpl(LocalDate.now(), 2, u.getContainers().getSquadrons().get(0), "Diversi"));
			u.addExcursion(new UscitaImpl(LocalDate.now(), u.getReparto(),"Bellissima"));
		} catch (Exception e1) {
			e1.printStackTrace();
		}
		System.out.println("Escursioni aggiunte: ");
	}
